module HolidaysHelper
  ...
  def render_holiday_tooltip(holiday)
    ...
    "<strong>#{@cached_label_start_date}</strong>: #{format_date(holiday.start_date)}<br />".html_safe +
    "<strong>#{@cached_label_due_date}</strong>: #{format_date(holiday.due_date)}<br />".html_safe  +
